...
同群同时一道题，点击 A～D 会发送 `/sound answer <题号> <选项>` 指令，也可以手动输入答题指令。首个有效作答立即结束本题，公布对错及正确答案；其他答案及原超时任务不能重复结算。无人作答时在截止后公布答案。群聊与私聊每次答对均向答题者发放 15 金粒，答错或超时不发放。奖励计入现有金粒余额，成功入账后记录来源 `sound_correct`；结算消息发送失败不会重复发奖，入账失败会明确提示。
...
第一步联网访问 Mojang 官方清单、元数据与资源节点，校验 SHA-1 并缓存到 `build/sound-cache/`；其余步骤离线运行。资源长期保存在 `doc/sounds/`，包含原版音频、播放副本和题库索引，清理 `build/` 不会删除这些资源。压缩包仍输出到 `build/minecraft-sound-26.3-rc-3.zip`，需要时可重新打包。完整执行后再上传；仅执行第一步尚未生成可玩的候选组。
