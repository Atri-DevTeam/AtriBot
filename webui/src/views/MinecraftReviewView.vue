<template>
  <div class="shell">
    <AppSidebar v-model:open="sidebarOpen" :app-id="appId" :bot-open-id="botOpenId" :bot-name="botName">
      <template #toolbar><button class="ghost-button" :disabled="loading" @click="refresh">刷新</button><button class="ghost-button" @click="logout">退出</button></template>
    </AppSidebar>
        ...
        <div class="mc-review-page">
          ...
          <section class="errors-surface mc-review-card">
            <header class="mc-review-head">
              ...
              <div class="mc-status-tabs"><button v-for="entry in statuses" :key="entry.value" :class="{active:status===entry.value}" @click="changeStatus(entry.value)">{{ entry.label }}</button></div>
              <form class="mc-search" @submit.prevent="runSearch"><input v-model.trim="searchInput" :placeholder="type==='names'?'搜索玩家名':'搜索 UUID 或玩家名'" :aria-label="type==='names'?'搜索玩家名':'搜索 UUID 或玩家名'" autocomplete="off"/><button class="ghost-button" :disabled="loading" type="submit">搜索</button><button v-if="searchTerm" class="ghost-button" type="button" @click="clearSearch">清除</button></form>
            </header>
...
async function fetchAllRecords(kind){
  ...
  let expected=0
  while(true){
    const q=new URLSearchParams({status:status.value,page:String(current),size:String(pageSize)})
    const data=await api(`/minecraft-moderation/${kind}?${q}`)
    const batch=Array.isArray(data?.items)?data.items:[]
    records.push(...batch)
    expected=Number(data?.total)||expected
    if(!batch.length||batch.length<pageSize||(expected>0&&records.length>=expected))break
    current++
  }
  return records
}
function normalizedUuid(value){return String(value||'').replaceAll('-','').trim().toLowerCase()}
function looksLikeUuid(value){return /^[0-9a-f]{32}$/i.test(normalizedUuid(value))}
function skinUuids(item){return [item.sampleUuid,item.uuid,item.playerUuid,item.minecraftUuid,item.ownerUuid].map(normalizedUuid).filter(Boolean)}
function filterSearch(records){
  const needle=searchTerm.value.toLowerCase()
  if(type.value==='names')return records.filter(item=>[item.name,item.originalName].some(value=>String(value||'').toLowerCase().includes(needle)))
  const uuid=normalizedUuid(searchUuid.value)
  return uuid?records.filter(item=>skinUuids(item).includes(uuid)):[]
}
async function load(){
  loading.value=true;error.value=''
  try{
    if(searchTerm.value){
      if(searchResults.value===null)searchResults.value=filterSearch(await fetchAllRecords(type.value))
      total.value=searchResults.value.length
      const start=(page.value-1)*size
      items.value=searchResults.value.slice(start,start+size)
    }else{
      const q=new URLSearchParams({status:status.value,page:String(page.value),size:String(size)})
      const data=await api(`/minecraft-moderation/${type.value}?${q}`)
      items.value=data?.items||[];total.value=Number(data?.total)||0
    }
  }catch(e){items.value=[];total.value=0;error.value=e.message}
  finally{loading.value=false}
}
async function runSearch(){
  searchTerm.value=searchInput.value.trim()
  searchResults.value=null;searchUuid.value='';page.value=1
  if(!searchTerm.value)return load()
  if(type.value==='skins'&&!looksLikeUuid(searchTerm.value)){
    loading.value=true;error.value=''
    try{
      const profile=await api(`/minecraft-moderation/profile?player=${encodeURIComponent(searchTerm.value)}`)
      if(!profile?.uuid)throw new Error('后端未返回玩家 UUID')
      searchUuid.value=profile.uuid
    }catch(e){items.value=[];total.value=0;error.value=e.message;loading.value=false;return}
    finally{if(!searchUuid.value)loading.value=false}
  }
  await load()
}
function clearSearch(){searchInput.value='';searchTerm.value='';searchUuid.value='';searchResults.value=null;page.value=1;load()}
function changeStatus(value){status.value=value;page.value=1;searchResults.value=null;if(searchTerm.value)runSearch();else load()}
function switchType(value){type.value=value;page.value=1;searchInput.value='';searchTerm.value='';searchUuid.value='';searchResults.value=null;load()}
function refresh(){if(searchTerm.value)searchResults.value=null;load()}
