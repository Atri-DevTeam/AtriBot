...
import top.yzljc.atribot.feature.classtable.ProcessClassTable;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        for (int dayOffset = 0; dayOffset <= LOOKAHEAD_DAYS; dayOffset++) {
            ...
            for (int session = startSession; session <= MAX_SESSION; session++) {
                int result = ProcessClassTable.getClassTableJson(session, sender.groupId(), date);
                if (result == ProcessClassTable.RESULT_FOUND) {
                    return true;
                }
                if (result == ProcessClassTable.RESULT_REQUEST_FAILED) {
                    sender.reply("课表接口暂时不可用，请稍后再试。", false);
                    return true;
                }
            }
        }

        sender.reply("近期未查询到课程安排。", false);
        return true;
    }
