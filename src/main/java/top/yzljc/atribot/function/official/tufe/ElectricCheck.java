...
public class ElectricCheck implements Listener, CommandExecutor {
                ...
                sender.sendMessage(TC.md(HELP_INFO),
                        TC.keyboard(
                                List.of(List.of(
                                                ...
                                                new Button("c2", "解绑", "/elec unbind", true, ButtonStyle.BLUE, ButtonType.COMMAND)
                                        ),
                                        List.of(
    ...
    public void onGroupMessage(OfficialGroupMessageCreateEvent event) {
        ...
        for (String k : KEYWORDS_ELECTRIC) {
            if (event.getMessage().getContent().trim().equalsIgnoreCase(k)) {
                ThreadManager.execute(() -> {
                    String queryResult = handlePrivateModule();
    ...
    private static Object getKeys(String dormRoomNum, String acRoomNum, String schoolRegion, boolean hasDorm, boolean hasAirCon, String lastCommand) {
        ...
        if (hasDorm) {
            bindButtons.add(new Button("c1", "宿舍电表", "/elec " + dormRoomNum + " " + schoolRegion + " 0", false, ButtonStyle.GRAY, ButtonType.COMMAND));
        }
        if (hasAirCon) {
            bindButtons.add(new Button("c2", "空调电表", "/elec " + acRoomNum + " " + schoolRegion + " 1", false, ButtonStyle.GRAY, ButtonType.COMMAND));
        }
        ...
        layout.add(List.of(
                new Button("c3", "再次查询", lastCommand, false, ButtonStyle.BLUE, ButtonType.COMMAND)
        ));
        layout.add(List.of(
                new Button("c4", "帮助信息", "/elec help", false, ButtonStyle.BLUE, ButtonType.COMMAND)
        ));
    ...
    private void handleBind(CommandSender sender, String[] args) {
        ...
        try {
            ...
            if (TufeElecRepository.bind(sender.getUserId(), roomNum, schoolRegion, type)) {
    ...
    private void handleUnbind(CommandSender sender, String[] args) {
        ...
        try {
            ...
            if (TufeElecRepository.unbind(sender.getUserId(), type)) {
                sender.sendMessage("解绑成功");
            } else {
                sender.sendMessage("当前未绑定该类型电表");
            }
    ...
    private void handleManualQuery(CommandSender sender, String[] args) {
        ...
        try {
            ...
            sendQueryResult(sender, roomNum, schoolRegion, type, "查询成功", "/elec " + roomNum + " " + schoolRegion + " " + type);

        } catch (Exception e) {
            sender.sendMessage("参数格式错误，请使用 /elec help 查看帮助");
        }
    }
    ...
    private void sendQueryResult(CommandSender sender, long roomNum, int schoolRegion, int type, String feedback, String command) {
        ...
        ElecDTO dormBind = TufeElecRepository.getDataByOpenIdAndType(sender.getUserId(), 0);
        ElecDTO acBind = TufeElecRepository.getDataByOpenIdAndType(sender.getUserId(), 1);
        ...
        sender.sendMessage(TC.md(info.replace("{placeholder_feedback}", feedback).replace("{placeholder_data}", queryResult)),
