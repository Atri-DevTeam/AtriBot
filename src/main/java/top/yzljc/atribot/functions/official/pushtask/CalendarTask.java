...
public final class CalendarTask extends PushTask {
    ...
    public Markdown enable(String groupOpenId, String operatorOpenId) {
        if (!GroupList.isAllowedFullMessages(groupOpenId)) {
            return FullMessageGroup.n();
        }

        GroupList.setFunctionEnabled(groupOpenId, this.getFunctionId(), true, operatorOpenId);
        return TC.md("✅ 已启用**" + this.getDisplayName() + "**");
    }

    @Override
    public Markdown disable(String groupOpenId, String operatorOpenId) {
        GroupList.setFunctionEnabled(groupOpenId, this.getFunctionId(), false, operatorOpenId);
