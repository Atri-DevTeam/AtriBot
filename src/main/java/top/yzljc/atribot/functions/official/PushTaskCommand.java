...
import top.yzljc.atribot.functions.official.pushtask.CalendarTask;
import top.yzljc.atribot.functions.official.pushtask.MinecraftNewsCheckTask;
import top.yzljc.atribot.functions.official.pushtask.PushTask;
...
public class PushTaskCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 1) {
            ...
            Object feedbackButton = Keyboard.compose(
                    List.of(
        ...
        if (args.length > 1) {
            ...
            switch (cmd) {
                ...
                case "开启" -> {
                    sender.sendMessage(task.enable(groupOpenId, sender.unionOpenId()), Keyboard.compose(List.of(List.of(back()))));
                    return true;
                }
                case "关闭" -> {
                    sender.sendMessage(task.disable(groupOpenId, sender.unionOpenId()), Keyboard.compose(List.of(List.of(back()))));
                    return true;
                }
            }
        }
