...
public class FullMessageEnableCommand implements CommandExecutor, Listener {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        Object keyboard = Keyboard.compose(
                List.of(
                        List.of(linkButton),
        ...
        GroupList.setRealGroupId(sender.groupOpenId(), Long.parseLong(groupRealId));
        return true;
    }
    ...
    public void onCallback(OfficialInteractionEvent event) {
        ...
        if (GroupList.isAllowedFullMessages(event.getGroupOpenId())) {
            event.answer(AnswerCode.REPEAT);
            return;
        }
        String messageId = event.sendMessage(TC.md("> 全量消息已开放，若后续检测到主动推送失败将自动关闭"));
        if (messageId != null) {
            GroupList.setAllowedFullMessage(event.getGroupOpenId(), true);
    ...
    public void onFullMessageFail(OfficialActiveMessageFailEvent event) {
        ...
        if (GroupList.isAllowedFullMessages(groupOpenId) && event.getErrorMessage().equals("主动消息失败, 无权限") && event.getGroupOpenId() != null) {
            GroupList.setAllowedFullMessage(groupOpenId, false);
        }
    }
}