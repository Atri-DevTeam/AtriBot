    ...
    public void onRunCommand(UserRunCommandEvent event) {
        if (event.getLabel().equals("2") && GroupList.isGroupBlacklisted(event.getSender().groupOpenId())) {
            if (!event.getCommandHeader().equalsIgnoreCase("feedback")) {
                ...
                event.getSender().replyMarkdown(event.getLabel(), "> 该群聊因违反指令使用规则已被禁止使用指令，如有任何疑问请联系开发者处理！", key);
        ...
        if (event.getLabel().equals("1") && PermissionGroup.hasRole(event.getSender().unionOpenId(), PermissionRole.BLACKLIST)) {
            if (!event.getCommandHeader().equalsIgnoreCase("feedback")) {
                Object key = Atri.getInstance().getChatService().buildCmdKeyboard(List.of(
                        List.of(new CommandButton("c1", "联系开发者", "/feedback", true, 1, 2))
                ));
                event.getSender().replyMarkdown(event.getLabel(), "> 你已被禁止使用指令，如有任何疑问请联系开发者处理！", key);
        ...
        if (event.getLabel().equals("2") && PermissionGroup.hasRole(event.getSender().unionOpenId(), PermissionRole.BLACKLIST)) {
            if (!event.getCommandHeader().equalsIgnoreCase("feedback")) {
                Object key = Atri.getInstance().getChatService().buildCmdKeyboard(List.of(
                        List.of(new CommandButton("c1", "联系开发者", "/feedback", true, 1, 2))
                ));
                event.getSender().replyMarkdown(event.getLabel(), "> 你已被禁止在群聊中使用指令，如有任何疑问请联系开发者处理！", key);
        ...
        if (event.getLabel().equals("1") || event.getLabel().equals("2")) {
