...
public class MinesweeperGame implements Listener, CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        String sessionId = label.equals("1") ? sender.unionOpenId() : sender.groupOpenId();
        ...
        if (action.equals("start")) {
            ...
            handleStartGame(sessionId, label, sender, mines);
            return true;
        }
        ...
        if (game == null) {
            sender.replyText(label, "当前没有正在进行的扫雷游戏喵，请发送 /扫雷 开始新游戏！");
            return true;
        }

        game.participants.add(sender.unionOpenId());
        ...
        synchronized (game) {
            if (game.isGameOver) {
                sender.replyText(label, "本局游戏已经结束了喵！请重新发送 /扫雷 开始新游戏！");
                return true;
            }

            if (action.equals("l")) {
                handleDig(game, row, col, sessionId, label, sender);
            } else if (action.equals("q")) {
                handleFlag(game, row, col, sessionId, label, sender);
            }
        }
        return true;
    }
    ...
    private void sendWelcomeScreen(String sessionId, String label, CommandSender sender) {
        if (activeGames.containsKey(sessionId)) {
            sender.replyText(label, "当前环境已经有一个正在进行的扫雷游戏了喵！请通关或等它结束后再开！");
            return;
        }
    ...
    private void handleStartGame(String sessionId, String label, CommandSender sender, int minesCount) {
        ...
        newGame.participants.add(sender.unionOpenId());
        activeGames.put(sessionId, newGame);

        sendOrUpdateGameBoard(newGame, sessionId, label, sender, "游戏已开始！");
    }

    private void handleDig(GameState game, int r, int c, String sessionId, String label, CommandSender sender) {
        // --- 非法操作提示：已插旗不允许挖开 ---
        if (game.flagged[r][c]) {
            sendOrUpdateGameBoard(game, sessionId, label, sender, "⚠️ 此处已插旗，无法挖开！请先拔旗。");
            return;
        }
        // --- 非法操作提示：已经挖开了 ---
        if (game.revealed[r][c]) {
            sendOrUpdateGameBoard(game, sessionId, label, sender, "⚠️ 已经挖开啦，不要重复挖喵！");
            return;
        }
        ...
        if (game.board[r][c] == -1) {
            ...
            game.isWin = false;
            revealAllMines(game);
            finishGame(game, sessionId, label, sender, "💥 踩到雷了！游戏失败！");
            return;
        }

        if (game.board[r][c] == 0) floodFill(game, r, c);

        checkWinCondition(game, sessionId, label, sender);
        if (!game.isGameOver) {
            sendOrUpdateGameBoard(game, sessionId, label, sender, "挖开了 " + (char) ('A' + r) + (c + 1));
        }
    }

    private void handleFlag(GameState game, int r, int c, String sessionId, String label, CommandSender sender) {
        // --- 非法操作提示：已经挖开了的格子不能插旗 ---
        if (game.revealed[r][c]) {
            sendOrUpdateGameBoard(game, sessionId, label, sender, "⚠️ 已经挖开的区域不能插旗喵！");
            return;
        }

        game.flagged[r][c] = !game.flagged[r][c];
        sendOrUpdateGameBoard(game, sessionId, label, sender, game.flagged[r][c] ? "成功插旗 🚩" : "已拔除旗帜 🔲");
    }
    ...
    private void checkWinCondition(GameState game, String sessionId, String label, CommandSender sender) {
        ...
        if (unrevealedCount == game.minesCount) {
            game.isGameOver = true;
            game.isWin = true;
            finishGame(game, sessionId, label, sender, "🎉 恭喜通关！");
        }
    }
    ...
    private void finishGame(GameState game, String sessionId, String label, CommandSender sender, String resultMsg) {
        ...
        sendKeyboardMessage(game, label, sender, markdown);
    }

    private void sendOrUpdateGameBoard(GameState game, String sessionId, String label, CommandSender sender, String actionMsg) {
    ...
    private void sendKeyboardMessage(GameState game, String label, CommandSender sender, String markdown) {

        List<List<CommandButton>> layout = new ArrayList<>();
        for (int r = 0; r < 5; r++) {
            List<CommandButton> rowBtn = new ArrayList<>();
            for (int c = 0; c < 5; c++) {
                ...
                rowBtn.add(new CommandButton(btnId, text, data, true, style, 2));
            }
            layout.add(rowBtn);
        }
