...
public class DiceImpl {
    ...
    public static void handle(CommandSender sender, String label, String[] args) {
        ...
        if (count > 1) {
            rollMultiple(sender, label, count);
        } else {
            rollSingle(sender, label);
        }
    }

    public static void rollSingle(CommandSender sender, String label) {
        ...
        String text = switch (rolledNumber) {
            case 7 -> "> Your High Class Archfiend Dice rolled a 7!\n" +
                    ...
                    "> WOW! " + At.at(sender.unionOpenId()).replace("\n", "") + "found Archfiend Dye!\n" +
