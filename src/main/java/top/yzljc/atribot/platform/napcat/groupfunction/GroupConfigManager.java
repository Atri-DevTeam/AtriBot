...
public class GroupConfigManager {
    ...
    private static final String CONFIG_FILE = Properties.GROUP_CONFIG;
    private static final ObjectMapper jsonMapper = new ObjectMapper().enable(SerializationFeature.INDENT_OUTPUT);
    private static Map<String, Map<String, Boolean>> groupConfigCache = new ConcurrentHashMap<>();
    @Getter
    ...
    public static synchronized void refreshAllConfigs() {
        ...
        Set<String> currentOnlineGroups = GroupInformation.fetchAllGroupIds();

        boolean isDebug = Config.getInstance().isDebugMode();

        boolean isUpdated = false;

        for (String groupId : currentOnlineGroups) {
        ...
        Iterator<String> iterator = groupConfigCache.keySet().iterator();
        while (iterator.hasNext()) {
            String cachedGroupId = iterator.next();
    ...
    public static boolean isFeatureEnabled(String groupId, String featureName) {
    ...
    public static synchronized void toggleFeature(String groupId, String featureName) {
        groupConfigCache.computeIfAbsent(groupId, _ -> new HashMap<>());
        Map<String, Boolean> settings = groupConfigCache.get(groupId);
    ...
    public static synchronized void setFeature(String groupId, String featureName, boolean enabled) {
        groupConfigCache.computeIfAbsent(groupId, _ -> new HashMap<>());
        Map<String, Boolean> settings = groupConfigCache.get(groupId);
    ...
    public static Map<String, Boolean> getStatusMapForFeature(String featureName) {
        Map<String, Boolean> statusMap = new HashMap<>();
        boolean def = registeredFeatures.getOrDefault(featureName, false);
        for (Map.Entry<String, Map<String, Boolean>> entry : groupConfigCache.entrySet()) {
    ...
    private static void loadConfigFromFile() {
        ...
        try {
            groupConfigCache = jsonMapper.readValue(file, new TypeReference<ConcurrentHashMap<String, Map<String, Boolean>>>() {});
        } catch (IOException e) {
