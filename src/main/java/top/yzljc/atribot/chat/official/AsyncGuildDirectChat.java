...
public final class AsyncGuildDirectChat {
     ...
     * @return 消息 ID，发送失败返回 null
     */
    public static CompletableFuture<String> replyMessage(String guildId, RT rt, Embed embed) {
        Objects.requireNonNull(rt, "被动消息中msg_id和event_id不能同时为空");
        return service().sendGuildDirectMessageAsync(guildId, service().getBodyFactory().embeds(rt, embed));
    }
}
