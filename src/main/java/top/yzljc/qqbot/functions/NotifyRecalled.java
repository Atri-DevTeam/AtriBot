    ...
    public void onGroupMessageRecall(RecallMessageEvent event) {
        ...
        if (foundMessage != null) {
            ...
            if (event.getUserId() != event.getOperatorId()) {
                toSend.add(GroupMessage.createTextNode("[群聊][" + time + "][" + groupName +  "][" + userName + "]的一条消息被群管理员" + GetUserInfo.getUserName(event.getOperatorId()) + "撤回: ", String.valueOf(event.getSelfId()), "AtriBot"));
